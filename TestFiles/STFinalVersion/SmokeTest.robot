...
${HomepageURL}                  https://puffy.com/?preview_theme_id=84017709090
...
Check current cart count (Royal Hybrid)
    ${CartText}   get text  xpath=/html/body/div[10]/div/header[2]/div/div/div/div[3]/div/a/span
    set test variable       ${CartCount}    ${CartText}
...
Check cart count after adding a mattress (Royal Hybrid)
    ${BeforeShopping} =     evaluate    ${CartCount}
    sleep                   2s
    Check Current Cart Count (Royal Hybrid)
    ${AfterShopping}=       evaluate    ${CartCount}-2
...
Select and proceed with Klarna
    ...
    sleep                               5s
    wait until page contains            ${KlarnaH1}
